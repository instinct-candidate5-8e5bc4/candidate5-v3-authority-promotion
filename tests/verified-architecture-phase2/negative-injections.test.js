...
test('multi-failure precedence is evidence before stale/contact/collision/containment and stable across runs',()=>{const m=model('school'),r=request(m,{evidenceRefs:[],surfaceId:'missing',transform:{x:5.4,y:.4,z:-3.84}});const runs=Array.from({length:20},()=>evaluate(r,m));assert(runs.every(x=>x.reasonCode==='GEOMETRY_EVIDENCE_MISSING_OR_MISMATCHED'));assert.equal(new Set(runs.map(JSON.stringify)).size,1);});
test('multi-failure precedence is stale before contact/collision/containment',()=>{const m=model('school'),r=request(m,{surfaceId:'floor',transform:{x:5.4,y:.4,z:-3.84}});r.surfaceModelRef.revision=2;const runs=Array.from({length:20},()=>evaluate(r,m));assert(runs.every(x=>x.reasonCode==='SURFACE_PROOF_STALE_OR_MISMATCHED'));assert.equal(new Set(runs.map(JSON.stringify)).size,1);});
test('multi-failure precedence is contact before door collision and containment',()=>{const m=model('school'),r=request(m,{surfaceId:'floor',transform:{x:5.4,y:.4,z:-3.84}});const runs=Array.from({length:20},()=>evaluate(r,m));assert(runs.every(x=>x.reasonCode==='CONTACT_GAP_FLOATING'));assert.equal(new Set(runs.map(JSON.stringify)).size,1);});
test('multi-failure precedence is door collision before containment',()=>{const m=model('school'),r=request(m,{surfaceId:'floor',transform:{x:5.4,y:.175,z:-3.84}});const runs=Array.from({length:20},()=>evaluate(r,m));assert(runs.every(x=>x.reasonCode==='DOOR_OR_OPENING_PENETRATION'));assert.equal(new Set(runs.map(JSON.stringify)).size,1);});
