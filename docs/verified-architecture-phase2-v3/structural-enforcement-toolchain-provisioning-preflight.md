...
The sealed cache and installed tree MUST preserve every license, copyright, notice, exception and third-party attribution shipped in each selected archive. Before cache admission, select (a) every regular file whose basename starts with `LICENSE`, `COPYRIGHT` or `NOTICE`, case-insensitively, and (b) recursively every regular file below any directory whose basename starts with one of those strings. Reject absolute, duplicate or escaping paths. The canonical relative path is the complete archive-internal member name exactly as stored, including the archive's top-level directory and using `/`; it excludes the local cache filename and extraction-root prefix. For each selected regular file, emit exactly `path UTF-8 bytes || NUL || decimal byte length with no leading zero || NUL || lowercase SHA-256 hex || LF`. Paths MUST already be valid UTF-8 and use `/`; no Unicode normalization, case folding, separator rewriting, dot-segment cleanup or extraction-root prefix is allowed. Reject NUL, backslash, empty segment, `.` or `..` segment, absolute path, non-UTF-8 path, duplicate raw path, normalized/case-fold collision, hard link, symlink, device, FIFO or any selected non-regular entry. Traverse all archive entries and all descendants of matched directories without following links. Sort complete records by unsigned raw UTF-8 path bytes and concatenate with no header, trailer or extra LF. An empty selection is invalid and MUST FAIL CLOSED. Then verify extracted and installed copies byte-for-byte against the manifest. Do not deduplicate required paths even when bytes match.

For the three authenticated Rust 1.98.1 archives, this rule selects 26 regular files. The complete canonical bytes are committed at `rust-1.98.1-license-notice-manifest.bin`; the path/digest inventory is committed at `rust-1.98.1-license-notice-inventory.md`. The 3,685-byte canonical manifest has SHA-256 `e01f11e6bdba74f2b87a36c86b75691682850ee6e8b3466c5c6f18166dff8200`. This includes all 12 regular files below `rustc-1.98.1-x86_64-unknown-linux-gnu/rustc/share/doc/rust/licenses/`, as well as the 14 directly matching regular files. Provisioning MUST regenerate and byte-compare the committed canonical manifest and reproduce its exact count, byte length and root from authenticated payloads; a different byte, count, length or root is `HARD STOP - LICENSE INVENTORY MISMATCH` and the evidence must explain the first differing record; the research root is a second equality check, not authority to omit raw notice files. The evidence bundle includes all raw notices, the manifest, detected license expressions/exceptions and a deterministic mapping from every cached/installed artifact to applicable notices. Missing, altered, unrecognized or undistributable terms MUST FAIL CLOSED and require legal/owner review; provisioning PASS makes no legal conclusion.
