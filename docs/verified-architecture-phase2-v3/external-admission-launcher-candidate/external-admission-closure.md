...
Canonical records are `path NUL role NUL decimal-length NUL lowercase-sha256 LF`, raw-path sorted. Mechanically derived from exact candidate command references and recursive ELF interpreter/DT_NEEDED resolution against the approved OCI root: 11 executables, 24 files, 2,781 bytes, SHA-256 `71dc47fe995b29822eccf9aa653466a76fe612c45991d573c0f05c7ed1afe26e`. Records name the actual external `/usr/bin` and `/usr/lib` namespace invoked by the artifact. Git plumbing commands are built into `/usr/bin/git`; no remote operation, hook, alias or pager is used.
