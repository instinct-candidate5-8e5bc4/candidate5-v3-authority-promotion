...
Canonical records are `path NUL role NUL decimal-length NUL lowercase-sha256 LF`, raw-path sorted. Mechanically derived from exact candidate command references and recursive ELF interpreter/DT_NEEDED resolution against the approved OCI root: 14 executables, 29 files, 3,332 bytes, SHA-256 `9e2b064a551e88fb4faa83bf8e169249d10a4ba8464ac19e2bf0d26bc7e6574c`. Git subcommands are built into `/usr/bin/git`; the candidate sets no external pager, hooks, aliases or remote operation.
