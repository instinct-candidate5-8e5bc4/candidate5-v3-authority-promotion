...
`external protected admission key/config -> signed external-admission binding -> exact admission artifact + 30-file closure -> exact b768d48 repository/commit/tree/script blob -> one opened script object -> descriptor verification/evidence -> same-descriptor Bash handoff`.
