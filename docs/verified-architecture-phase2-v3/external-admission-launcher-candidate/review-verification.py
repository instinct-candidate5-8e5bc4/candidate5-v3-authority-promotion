...
s=art.read_text();assert 'case "$0" in /proc/self/fd/[0-9]*' in s and 'admission_fd_object=' in s and 'admission_path_object=' in s and 'E_ADMISSION_DESCRIPTOR_OBJECT' in s;assert '$STAGE' not in s and 'admission-message.tmp' not in s;assert s.index('exec 9<"$SCRIPT"')<s.index('sha256sum "$fd"')<s.index('exec /usr/bin/bash --noprofile --norc "$fd"');assert s.index('fail E_CANDIDATE_AUTHORITY')<s.index('>"$EVIDENCE_STAGE/admission.v1.txt"')<s.index('exec /usr/bin/bash')
rows=[]
...
host=json.loads((A/'hostile-fixtures/expected-results.v1.json').read_text());assert len(host['cases'])==21
