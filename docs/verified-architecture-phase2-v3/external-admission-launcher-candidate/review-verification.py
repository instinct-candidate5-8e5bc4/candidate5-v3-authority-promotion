...
s=art.read_text();assert '$STAGE' not in s and 'admission-message.tmp' not in s;assert s.index('exec 9<"$SCRIPT"')<s.index('sha256sum "$fd"')<s.index('exec /usr/bin/bash --noprofile --norc "$fd"');assert s.index('fail E_CANDIDATE_AUTHORITY')<s.index('>"$EVIDENCE_STAGE/admission.v1.txt"')<s.index('exec /usr/bin/bash')
rows=[]
