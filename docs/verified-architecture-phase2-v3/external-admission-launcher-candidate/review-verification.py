...
assert x['closure']=={'blob':blob(closure),'byteLength':len(closure.read_bytes()),'pathCount':24,'sha256':h(closure)}
assert x['script']['blob']=='2622618addebf7385bda722295965540cf51c849'
s=art.read_text();assert '$STAGE' not in s and 'admission-message.tmp' not in s;assert s.index('exec 9<"$SCRIPT"')<s.index('sha256sum "$fd"')<s.index('exec /usr/bin/bash --noprofile --norc "$fd"');assert s.index('fail E_CANDIDATE_AUTHORITY')<s.index('>"$EVIDENCE/admission.v1.txt"')<s.index('exec /usr/bin/bash')
rows=[]
for line in closure.read_bytes().splitlines():
 p,r,n,d=line.split(b'\0');rows.append(p)
assert len(rows)==24 and rows==sorted(rows) and len(set(rows))==24
host=json.loads((A/'hostile-fixtures/expected-results.v1.json').read_text());assert len(host['cases'])==18
