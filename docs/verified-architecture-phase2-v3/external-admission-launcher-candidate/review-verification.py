...
assert x['closure']=={'blob':blob(closure),'byteLength':len(closure.read_bytes()),'pathCount':29,'sha256':h(closure)}
assert x['script']['blob']=='2622618addebf7385bda722295965540cf51c849'
s=art.read_text();assert s.index('trap cleanup EXIT')<s.index('/usr/bin/mkdir -m 700 "$STAGE"');assert s.index('exec 9<"$SCRIPT"')<s.index('sha256sum "$fd"')<s.index('exec /usr/bin/bash --noprofile --norc "$fd"');assert s.index('fail E_CANDIDATE_AUTHORITY')<s.index('exec /usr/bin/bash')
rows=[]
for line in closure.read_bytes().splitlines():
 p,r,n,d=line.split(b'\0');rows.append(p)
assert len(rows)==29 and rows==sorted(rows) and len(set(rows))==29
host=json.loads((A/'hostile-fixtures/expected-results.v1.json').read_text());assert len(host['cases'])==15
