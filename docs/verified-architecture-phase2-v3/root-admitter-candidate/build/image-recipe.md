...
2. Create an 64 MiB zero file. Run exact `/usr/sbin/mke2fs` 1.46.5 under pinned libfaketime at `1970-01-01T00:00:01Z` with `E2FSPROGS_FAKE_TIME=1`, ext4, 4096-byte blocks, 256-byte inodes, fixed UUID `...0001`, fixed hash seed `...0002`, lazy initialization off, journal and metadata checksum seed disabled, and `-d rootfs`.
