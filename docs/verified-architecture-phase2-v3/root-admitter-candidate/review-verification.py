...
src=(R/'measured-supervisor.c').read_text();assert 'execve(args[0],args,env)' in src and '"PATH=/usr/bin","LANG=C","LC_ALL=C","TZ=UTC","PWD=/",NULL' in src;assert m['rootHash'] not in src and 'v3.root_admitter_verity=%s' in src and 'DM_READONLY_FLAG' in src and 'O_NOFOLLOW' in src and 'field==8' in src and 'chroot("/verity-root")' in src
