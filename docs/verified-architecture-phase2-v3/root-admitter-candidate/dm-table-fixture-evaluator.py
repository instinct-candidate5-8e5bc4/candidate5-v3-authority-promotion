...
import struct,sys
b=Path(sys.argv[1]).read_bytes();CAPACITY=24576
assert len(b)>=40
start,length,status,nxt,typ=struct.unpack_from('<QQiI16s',b)
assert start==0 and length==196608 and typ.rstrip(b'\0')==b'verity'
...
assert fields[0]==b'1' and fields[3:8]==[b'4096',b'4096',b'24576',b'0',b'sha256'] and len(fields[8])==64 and fields[9]==b'-'
