...
def record(label,p):
 data=p.read_bytes();return {'path':label,'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest(),'gitBlob':git_blob(data),'gitMode':mode(p)}
# Bind the complete certification closure. Generated self-referential records are handled below.
exclude={'inventory.v1.json','successor-authority-record.v1.json','successor-initramfs.cpio','successor-unsigned.efi'}
source=[]
for p in sorted(HERE.rglob('*'),key=lambda x:x.relative_to(HERE).as_posix().encode()):
 if p.is_file() and '__pycache__' not in p.parts and p.name not in exclude:
  source.append(record(p.relative_to(HERE).as_posix(),p))
ra=HERE.parent/'root-admitter-candidate'
for p in sorted(ra.rglob('*'),key=lambda x:x.relative_to(ra).as_posix().encode()):
 if p.is_file() and '__pycache__' not in p.parts:
  source.append(record('../root-admitter-candidate/'+p.relative_to(ra).as_posix(),p))
generated=[record(n,OUT/n) for n in ('successor-initramfs.cpio','successor-unsigned.efi')]
inv={'schema':'v3.successor-uki-candidate-inventory.v3','status':'CERTIFICATION_EVIDENCE_COMPLETE_RUNTIME_PASS','artifacts':source+generated,'initramfsMembers':member_vector,'preserved':{'cmdlineSha256':sha(OLD/'cmdline'),'kernelSha256':sha(OLD/'vmlinuz'),'oldInitSha256':hashlib.sha256(old_init['body']).hexdigest(),'rootAdmitterSha256':sha(OLD/'initramfs'/'bin'/'measured-supervisor'),'verityRoot':'533d6d61d83ad8e03539549d500fb74ec6d844f18ee3bcaec238f7fb78303245'},'certification':{'positiveTerminal':'E_CANDIDATE_AUTHORITY','positiveInterpretation':'PASS_AT_DESIGNED_PRE_MUTATION_AUTHORITY_BOUNDARY','hostileExecutableCases':12,'exactSetExclusions':['.v3-volume-role','lost+found']},'externalBinding':{'domain':'V3-SUCCESSOR-UKI-EXTERNAL-BINDING:v1','rule':'The adjacent canonical authority record binds this inventory, all certification closure records, and generated successor artifacts by path, length, SHA-256, Git blob and mode.'}}
(OUT/'inventory.v1.json').write_text(json.dumps(inv,sort_keys=True,separators=(',',':'))+'\n')
records=source+[record('inventory.v1.json',OUT/'inventory.v1.json')]+generated
binding={'schema':'v3.successor-uki-external-binding.v2','status':'CERTIFICATION_EVIDENCE_COMPLETE_RUNTIME_PASS','domain':'V3-SUCCESSOR-UKI-EXTERNAL-BINDING:v1','records':records,'certification':inv['certification'],'signingRule':'productionSignature = RSA-PSS-SHA256(privateKey, SHA256(UTF8(domain) || 0x00 || canonicalRecordWithoutSignature)); canonical JSON is sorted-key compact UTF-8 plus LF; signature and certificate DER are adjacent public artifacts and not fields in the signed core'}
