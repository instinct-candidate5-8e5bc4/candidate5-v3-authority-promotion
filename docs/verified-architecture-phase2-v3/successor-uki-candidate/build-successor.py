...
inv={'schema':'v3.successor-uki-candidate-inventory.v2','status':'SUCCESSOR_UKI_CONCRETE_CANDIDATE_READY_FOR_OWNER_REVIEW','artifacts':{n:{'bytes':((HERE/n) if n in ('cloud-boot-adapter.sh','build-successor.py','builder-runtime.v1.json','hostile-test-plan.v1.json','verify-candidate.py') else (OUT/n)).stat().st_size,'gitMode':'100755' if n in ('cloud-boot-adapter.sh','build-successor.py','verify-candidate.py') else '100644','sha256':sha((HERE/n) if n in ('cloud-boot-adapter.sh','build-successor.py','builder-runtime.v1.json','hostile-test-plan.v1.json','verify-candidate.py') else (OUT/n))} for n in ['cloud-boot-adapter.sh','build-successor.py','builder-runtime.v1.json','hostile-test-plan.v1.json','verify-candidate.py','successor-initramfs.cpio','successor-unsigned.efi']},'initramfsMembers':member_vector,'preserved':{'cmdlineSha256':sha(OLD/'cmdline'),'kernelSha256':sha(OLD/'vmlinuz'),'oldInitSha256':hashlib.sha256(old_init['body']).hexdigest(),'rootAdmitterSha256':sha(OLD/'initramfs'/'bin'/'measured-supervisor'),'verityRoot':'c8f1ca4197a6982aa09ea53dd92ef76fded30dce84c75ff63d2e978b4d227015'},'externalBinding':{'domain':'V3-SUCCESSOR-UKI-EXTERNAL-BINDING:v1','rule':'At signing review, freeze Git commit/tree and blob IDs/modes for README.md, build-successor.py, cloud-boot-adapter.sh and inventory.v1.json; bind their SHA-256 values plus unsigned/signed UKI SHA-256 and public certificate DER SHA-256 in one canonical record. The retained production key signs sha256(domain NUL || canonical record). Firmware authenticates embedded UKI bytes; this detached signature authenticates external evidence.'}}
...
bind_names=['README.md','build-successor.py','builder-runtime.v1.json','cloud-boot-adapter.sh','hostile-test-plan.v1.json','verify-candidate.py','inventory.v1.json','successor-initramfs.cpio','successor-unsigned.efi']
...
for n in bind_names:
 src=(HERE/n) if n in ('README.md','build-successor.py','builder-runtime.v1.json','cloud-boot-adapter.sh','hostile-test-plan.v1.json','verify-candidate.py') else (OUT/n)
 data=src.read_bytes(); binding['records'].append({'path':n,'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest(),'gitBlob':git_blob(data),'gitMode':'100755' if n in ('build-successor.py','cloud-boot-adapter.sh','verify-candidate.py') else '100644'})
