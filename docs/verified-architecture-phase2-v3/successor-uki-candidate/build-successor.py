...
for n in bind_names:
 ...
 data=src.read_bytes(); binding['records'].append({'path':n,'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest(),'gitBlob':git_blob(data),'gitMode':'100755' if n in ('build-successor.py','cloud-boot-adapter.sh','provider-producer/produce-google-by-id.sh','provider-producer/lib/udev/scsi_id','run-producer-vm.sh','qemu-extracted-closure.v1.json','vm-test/virtio_scsi.ko','vm-test/guest-init.sh','producer-vm-raw-serial.txt','producer-test-initramfs.cpio','build-producer-test-initrd.py','run-producer-vm.sh','qemu-extracted-closure.v1.json','vm-test/virtio_scsi.ko','vm-test/guest-init.sh','producer-vm-raw-serial.txt','producer-test-initramfs.cpio','build-producer-test-initrd.py','verify-candidate.py') else '100644'})
