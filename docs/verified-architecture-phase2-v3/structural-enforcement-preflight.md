...
The independently hosted revocation service has a separate offline-root-authorized signing key pinned in the guard. It signs `text("V3REVOCATION:v2") || text(repositoryId) || git20(artifactCommit) || git20(certificationCommit) || u64(pinConfigSequence) || sha256(pinTupleBytes) || sha256(certificationRoot) || u64(sequence) || u64(issuedUnixMs) || u64(expiresUnixMs) || sha256(entries)`. Namespace is `(repositoryId, artifactCommit, certificationCommit, pinConfigSequence, pinTupleDigest, certificationRoot)`. The minimum accepted revocation sequence and pin-tuple sequence/digest are stored in protected trusted verifier state, every guard result and signed release metadata; use the greater value. Max checkpoint age is 5 minutes and expiry may not exceed issue time by 10 minutes. Missing, stale, expired, rolled-back, equivocating, unknown-key or unavailable state fails.

### Atomic external pin state

The two commit IDs are one authenticated atomic configuration, never two environment variables. Central guard configuration stores a signed tuple with exact bytes: `text("V3BOUNDARYPINS:v1") || text(repositoryId) || u64(configSequence) || git20(sourceCommit) || git20(artifactCommit) || git20(certificationCommit) || git20(artifactTree) || sha256(rootSubstitutedBytes) || u64(issuedUnixMs)`. An offline-root-authorized configuration key, distinct from release/revocation keys and pinned in the installed guard, signs it. The trusted installer stores the tuple as one atomic signed object and uses all fields from that object. Partial fields or mixed sequences are rejected. An external transparency log may be added as defense-in-depth but is not authority in this reduced threat model.

The trusted verifier state carries the minimum `configSequence`; release metadata and revocation state also carry it, and verification uses the greatest observed floor. Updates require sequence +1, correct first-parent/tree equations, fresh approval/review and a new signed tuple. Protocol rollback requires a recovery signature naming current sequence/digest and target digest; it records a new higher sequence and never lowers the trusted floor. Tests cover atomic replacement, torn/mixed reads, replay of both old pins together, sequence skip/rollback, equivocation, wrong repository/source/tree/root and recovery rollback.

### External certification with NO PAID GITHUB PLAN DEPENDENCY

GitHub Free, repository refs, merge state, branch settings and GitHub Actions are untrusted publication/convenience surfaces. They MAY run defense-in-depth checks and display results, but they never grant authority. GitHub Free is allowed. GitHub Team/Enterprise, organization migration, protected environment, required workflow or ruleset is not required. Administrator action, branch protection failure, a green/forged check, merge, tag or moved ref cannot change the certified verdict.

The external verifier/guard with NO PAID GITHUB PLAN DEPENDENCY, named `structural-boundary-policy-guard` in evidence, is installed with the certified runtime baseline rather than enforced by GitHub. The sole authority predicate is `CERTIFICATION_VALID`. A candidate is authorized only when a separately installed verifier, whose binary and trust roots are part of the certified runtime baseline, validates this complete external chain from content-addressed bytes:

`deterministic external certification -> signed artifact/root -> atomic pin tuple -> certified runtime baseline`.

The verifier receives no authority from GitHub APIs. It fetches repository objects only as untrusted bytes by exact object ID, reconstructs every digest locally, retrieves the signed pin/revocation/checkpoint state and compares it with rollback floors protected by the trusted verifier/deployment state. Runtime evaluation accepts only an in-memory `CERTIFICATION_VALID` capability minted by this verifier for one exact candidate digest and session. A boolean/string supplied by application code, CI, GitHub, an admin, a file in the candidate, or a moved ref is not this capability.

#### Reduced guarantee A: trusted deployment root

This design chooses reduced guarantee A. In scope and potentially hostile are GitHub administrators, branch/ref manipulation, manual/admin merge bypass, forged/bypassed CI/status, unauthorized repository content, uncertified commits/artifacts, graph/bundle/artifact substitution, stale/revoked certification, rollback within the certification protocol and unauthorized routing edges.

Trusted and outside the claim are the deployment OS, privileged deployment/host administrator, initially installed verifier/bootstrap, verifier root material installed through trusted deployment, protected verifier sequence/rollback state, and integrity of the trusted execution environment. The design does not claim resistance to privileged compromise of this TCB, malicious full trusted-host/state restore, replacement of installed verifier/bootstrap, or OS/root compromise. TPM, remote attestation, a paid HSM and online authorization are optional only if a later threat model expands this boundary; none is required here.

Core property: "Compromise or bypass of GitHub repository governance, GitHub CI, branch/ref controls, or repository administration alone cannot cause an uncertified build to become an authorized runtime baseline, provided the trusted deployment environment and installed certification verifier remain uncompromised."

Initial bootstrap is a trusted installation ceremony, not self-authentication. The privileged deployment custodian authenticates and installs the verifier/bootstrap package, verifier root keys, initial monotonic rollback/sequence state, and exact initial certified runtime baseline through the trusted deployment channel. The installation manifest binds package and baseline identities. The trusted OS installer verifies it, writes package/state to privileged non-application-writable storage and atomically activates it. Package self-hashes are diagnostic; authority comes from the trusted ceremony and installed root material. Failure leaves no authorized runtime.

#### Authenticated runtime-baseline update protocol

The current trusted minimal updater is the sole verifier and state-transition writer; successor self-attestation has zero authority. Successor self-attestation has zero authority.

Normative codec: domains and schema strings are the exact ASCII bytes shown inside `text`; `text = u32be(byteLength)||shortest-form UTF-8`; `raw32` is 32 bytes without a length; `keyId` is `text` matching `[a-z0-9._-]{1,128}`; `u8/u16/u32/u64` are unsigned big-endian. Record framing is `u32be(totalBodyLength)||body`; the length prefix is not signed, while the exact body is the signature input. There are no optional fields. Decoders reject missing, unknown, duplicate or reordered semantic fields, bad enum/key ID/UTF-8, overlong length, trailing bytes and cross-domain/schema use. JSON is never a signature input. Independent encoder/decoder implementations consume the semantic field table and must emit/accept one byte string only.

`V3RUNTIMEUPDATE:v2` exact body order: domain/schema/deployment/repository; predecessor `u64` sequence then five raw32 baseline/verifier/bootstrap/updater/keySet digests; successor `u64` sequence then the same five digests; seven raw32 candidate/graph/bundle/policy/audit/healthTestContract/expectedHealthResult roots; `u32` keyTransitionCount and sorted transitions; three `u64` certification/pin/revocation floors; issue/expiry `u64`; raw32 nonce and payload root. Transition is `u8(role)||raw32(oldKey)||raw32(newKey)||u8(action)`, roles 1 certification/2 update/3 recovery/4 revocation, actions 1 retain/2 rotate/3 revoke, sorted unique by role. Current trusted update key signs the exact body under its domain.

`V3UPDATEHEALTH:v1` exact body order: domain/schema/deployment/repository; raw32 nonce, updateDigest, candidateRoot, successorExecutable, successorVerifier, successorUpdater, successorKeySet; predecessor `u64` sequence+raw32 baseline; successor sequence+baseline; raw32 graph/bundle/policy/audit/healthTestContract/observedResult roots; issue/expiry `u64`; `u8(predicate=1 PASS)`. Health signer is the predecessor/current trusted updater key. Exact PASS requires every binding equal pending update, fresh time, complete matching observer roots/tests, no effects and predecessor protected state still authoritative.

`V3KEYRECOVERY:v1` exact body order: domain/schema/deployment/repository; predecessor `u64` sequence+raw32 baseline; successor sequence+baseline; raw32 updateDigest/candidateRoot/certificationRoot; `u64` recoverySequence; raw32 nonce/currentKeySet/successorKeySet; `u32` revokedKeyCount and sorted raw32 keys; threshold `u16 M,u16 N`; three floor `u64`; issue/expiry; raw32 targetUpdateDigest; `u8(reason)`. Exact body requires M unique sorted, currently trusted, unrevoked recovery signatures plus current update signature; key rotation also needs new-key proof-of-possession. Recovery cannot skip certification, health or atomic commit.

Positive semantic update record: deployment `d`, repository `r`; predecessor 1, successor 2; all raw32 bindings zero; no key transitions; floors 1; issue 0; expiry 1. Its exact 723-byte body and SHA-256 appear below; signature input is exactly that body. A signature envelope is `text("V3SIGNATURE:v1") || text(keyId) || u8(algorithm=1 Ed25519) || raw64(signature)` and is external to body/hash. Signature verification chooses only the currently trusted update-key ID and domain.

Health raw observers are the current trusted updater's: staged-byte digest reader; process-image/executable digest reader; read-only candidate/update manifest reader; isolated test-harness observer; exit/signal/timeout observer; and protected-state observer proving no candidate writes. Required observations are exact successor executable/bootstrap/verifier/updater/key-set and candidate roots; predecessor/successor sequences; update digest/nonce; every health test ID/input/result/evidence digest; no missing/extra test; zero forbidden effects; bounded exit; and protected state still predecessor-authoritative. Observer records sort by test ID and encode `text(testId)||sha256(input)||u8(result=1 PASS)||sha256(evidence)`. Health-test-contract/result roots cover the ordered raw observers. Candidate output is never an observer. The current updater alone compares observations to update metadata, forms/signs `V3UPDATEHEALTH:v1`, and returns PASS only if all bindings, time and effects match.

Crash-consistent transition table for CURRENT, STAGED, HEALTH_PENDING, COMMIT_READY, COMMITTED, ROLLBACK/RECOVERY:

| Current -> next | Authority before/after | Durable writes and fsync boundary | Current updater decision | Crash restart |
| --- | --- | --- | --- | --- |
| `CURRENT -> STAGED` | predecessor executable/root/keys/floors both sides | write inactive content; fsync files/slot dir; append authenticated STAGED record; fsync journal+dir | update body/signature/reproduction valid | before record CURRENT; after record STAGED, predecessor authoritative |
| `STAGED -> HEALTH_PENDING` | predecessor remains sole authority | append HEALTH_PENDING with nonce/test contract; fsync journal+dir | staged digests re-read equal update | STAGED or HEALTH_PENDING; both predecessor |
| `HEALTH_PENDING -> COMMIT_READY` | predecessor remains sole authority; successor floors/keys inert | write current-updater-signed health+observer root; fsync; append COMMIT_READY; fsync journal+dir | trusted observations satisfy exact predicate | pending or ready; both predecessor |
| `COMMIT_READY -> COMMITTED` | one switch from complete predecessor tuple to complete successor tuple | construct one authenticated commit record containing slot/root/keySet/floors/sequence; write temp, fsync, atomic rename, fsync directory | reverify update, health, nonce, floors and current record | absent/torn commit: predecessor; complete valid commit: successor |
| any precommit -> `ROLLBACK` | predecessor remains authoritative | append failed nonce/reason while retaining predecessor tuple; fsync journal+dir; erase inactive slot later | failure/crash/timeout | deterministic predecessor + replay-denied nonce |
| `COMMITTED -> RECOVERY -> COMMITTED` | committed tuple remains until one higher-sequence recovery commit atomically replaces it | stage authorized older/new bytes; same health path; one atomic commit record | valid recovery threshold plus full certification/update/health | old committed tuple or fully recovered higher-sequence tuple |

There is no separate executable selector outside the authenticated tuple. Journal record fields include state enum (`1 CURRENT,2 STAGED,3 HEALTH_PENDING,4 COMMIT_READY,5 COMMITTED,6 ROLLBACK,7 RECOVERY`), active/inactive slot IDs, complete authoritative tuple, pending tuple, floors, nonce/failure set digest, record sequence and prior hash. Startup scans contiguous authenticated records, ignores incomplete tail and selects exactly the last valid state. Floors, roots, keys and executable change in the same COMMITTED record only. `COMMIT_READY` health signer remains the predecessor updater.

Hostile codec vectors and exact decoder results: swapped fields=`FIELD_ORDER`; duplicate semantic field in source object=`DUPLICATE_FIELD`; omitted field/short body=`MISSING_FIELD`; extra field/trailing byte=`UNKNOWN_OR_TRAILING`; overlong UTF-8=`NON_CANONICAL_UTF8`; wrong count/length=`LENGTH_MISMATCH`; update body under health/recovery domain=`CROSS_DOMAIN`; unknown enum/key role/action=`UNKNOWN_ENUM`; unordered/duplicate transition, observer, revoked key or signer=`NON_CANONICAL_ORDER`; wrong signature key/domain=`SIGNATURE_SCOPE`. State hostile vectors cover candidate-signed health, successor-key health, mismatched candidate/update/executable/root/keySet/sequence/nonce, expired health, mixed tuple commit, successor floor before commit, crash at every listed fsync/rename, failed nonce replay, recovery below threshold, duplicate/revoked signer, wrong predecessor/successor/update/candidate/certification binding, floor decrease and recovery bypass attempt; each is rejected with predecessor authority unchanged.

Canonical bound positive vectors use `deploymentId=d`, `repositoryId=r`, zero raw32 values, predecessor 1, successor 2, no transitions/revocations, floors 1, issue 0, expiry 1, health PASS, recovery 2-of-3/reason 1. These exact bodies implement the final schemas above:

- update 723 bytes, SHA-256 `1ff82930ec3120b7d1e8af51d0873b8fd5bd1d2a585f1611cf549516639fda36`, hex ``
- health 566 bytes, SHA-256 `33c413bd5b3bf2908a27659e76e1aec48a31af80dc41cd8e92e37e3ef0675015`, hex ``
- recovery 412 bytes, SHA-256 `ad5d42280aab2bd983fd5c644b53194c5b4eaf2d31e1272fcbf8f90057ee3928`, hex ``

Independent recovery decode assertion consumes exactly 412/412 bytes and returns `M=2`, `N=3`, floors `1/1/1`, issue `0`, expiry `1`, zero targetUpdateDigest and reason `1`; any trailing or missing byte fails. These are checked-in design evidence, not authorizing signatures. Independent encoders must reproduce exact bytes/hashes. Decoder hostile vectors and crash simulator outputs must be byte-identical across two clean runs. The later certified vectors add nonzero key transition, threshold signature and revocation cases without changing schemas.
...
- certified runtime baseline identity: monotonic runtime-baseline sequence, native installer/updater/verifier/bootstrap binaries, installed certification/update/recovery keys, trusted rollback/floor state, clock source, revocation state, SES/Endo/engine/OS target and protocol version.
...
CERTIFICATION_VALID = signatureValid
  ...
  && pinSequenceAtOrAboveTrustedFloor
...
| GitHub admin bypass | repository/GitHub admin disables checks, pushes directly, rewrites branches/tags, manually merges or publishes a release; trusted verifier ignores governance state and returns `NOT_AUTHORIZED` without valid certification. Privileged host/OS/bootstrap compromise is out of scope |
...
3. Approve compiler provenance/reproducibility, reduced-guarantee trusted deployment assumptions, enrollment/update/notary/floor controls, external certification/verifier and key/store/clock controls, GitHub-governance hostile proofs, external Web root, SES/Endo ABI, exact engine builds, and protocol limits.
