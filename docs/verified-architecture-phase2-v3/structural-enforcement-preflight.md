...
### External certification with no paid GitHub governance dependency
...
This design chooses reduced guarantee A. The deployment OS administrator, installed native supervisor, verifier/bootstrap bytes, OS/Node/engine install root, protected local key/floor storage and independent trust-root/checkpoint channel are trusted and outside the adversary claim. A party that can replace the verifier, rewrite its trust roots/floors, alter the native supervisor/OS, or counterfeit the independent channel can defeat this design. The hostile `admin bypass` proof covers repository/GitHub administration only. It MUST NOT claim resistance to deployment administrators, root/OS compromise, disk-image rollback with channel compromise, measured-boot compromise or physical replacement.

Initial enrollment is an explicit ceremony before runtime can start. Two deployment custodians obtain the same signed enrollment manifest over two independent authenticated channels, compare its digest and sequence, and install a content-addressed native supervisor package. The manifest binds repository ID, runtime-baseline sequence 1, verifier/bootstrap/supervisor/SES/Endo/engine/OS hashes, certification/pin/revocation/owner-checkpoint public keys, independent notary endpoint/key, trusted-clock endpoints/keys and initial minimum floors. An offline recovery-root signature and an online notary signature are both required. The trusted OS package manager verifies package hash/signatures, writes package plus manifest to root-owned non-application-writable storage, and atomically activates them. Self-hashes inside the package are diagnostic only; authority starts from the custodian-authenticated recovery key and independent notary key, not bundled claims. Failure leaves no enrolled runtime and yields `NOT_AUTHORIZED`.

The independent notary is the non-substitutable rollback anchor assumed by this reduced guarantee. Its public key is authenticated during enrollment and protected by the trusted deployment root. It maintains append-only namespaces for `(deploymentId, repositoryId, runtimeBaseline)` and signs monotonic runtime, pin and revocation sequence floors plus tree-head/inclusion proofs. Every authorization session fetches a fresh notary checkpoint over a separately pinned authenticated connection and uses `max(localFloor, notaryFloor, artifactFloor)`. A restored local snapshot remains below the notary floor and is rejected. Notary unavailability, stale checkpoint, invalid inclusion, equivocation or key mismatch is `NOT_AUTHORIZED`. If both deployment root and independent notary are rewritten, that is outside the claim.

#### Authenticated runtime-baseline update protocol

Every update is one signed `V3RUNTIMEUPDATE:v1` object binding deployment/repository IDs, predecessor and successor runtime-baseline sequences/digests, predecessor/successor verifier/bootstrap/supervisor/toolchain identities, all trust-key changes, minimum pin/revocation floors, issue/expiry, and update nonce. It requires signatures from the currently installed update key, offline recovery root and independent notary; key rotation additionally requires old and new key signatures. The predecessor must equal the active baseline exactly, successor sequence must be predecessor + 1, and all floors are monotonic. A recovery rollback is a new higher-sequence successor that names an older content digest plus incident authorization; it never lowers sequence/floors.

The trusted native supervisor downloads update bytes into an inactive content-addressed slot, verifies the whole object with the old verifier and a minimal immutable updater, reproduces required self-tests/hostile probes, obtains a fresh notary checkpoint, and writes package, tuple, floors and activation record in one OS-atomic transaction. It then starts the successor in quarantine; the successor re-verifies predecessor/successor binding and notary floor before an atomic slot flip. Crash or failure before the flip keeps the predecessor active. Failure after the flip but before a signed health record triggers fail-stop, not automatic downgrade. Recovery requires a separately signed higher-sequence update. Application code cannot request, select or activate updates.

Before any new verifier handles candidate bytes, the old trusted supervisor/updater validates the update chain and minimum runtime sequence. Replayed, skipped, mixed, partially written, expired, wrong-predecessor, floor-lowering, unsigned, revoked or downgraded updates are rejected. Conformance tests cover first install, wrong enrollment channel/key, duplicate custodian disagreement, valid successor, torn write, crash at each activation phase, failed successor health, replay, snapshot restore, key rotation, recovery-to-old-content-at-higher-sequence, notary outage/equivocation/skew and attempted downgrade before new-verifier execution.
...
- certified runtime baseline identity: monotonic runtime-baseline sequence, native supervisor/updater/verifier/bootstrap binaries, enrollment/update/recovery/notary keys, local floor store, independent notary/checkpoint channel, clock sources, transparency/revocation stores, SES/Endo/engine/OS target and protocol version.
...
| repository-admin bypass | repository/GitHub admin disables checks, pushes directly, rewrites branches/tags or publishes a release; external verifier ignores governance state and returns `NOT_AUTHORIZED` without valid chain. Deployment-admin/OS compromise is explicitly outside this reduced guarantee |
...
3. Approve compiler provenance/reproducibility, reduced-guarantee trusted deployment assumptions, enrollment/update/notary/floor controls, external certification/verifier and key/store/clock controls, repository-governance hostile proofs, external Web root, SES/Endo ABI, exact engine builds, and protocol limits.
