...
Every update is a deterministic signed `V3RUNTIMEUPDATE:v2` object binding deployment/repository IDs; exact authorized predecessor baseline/version/sequence; successor baseline/version/sequence; predecessor/successor verifier/bootstrap/toolchain identities; root-key transition; minimum certification, pin and revocation floors; issue/expiry; and update nonce. The currently trusted update root signs successor metadata. Root-key transition additionally requires old-root and new-root signatures plus an explicit recovery authorization. Predecessor must equal active trusted state, successor sequence is predecessor + 1 and all floors are monotonic. Replayed, skipped, wrong-predecessor, mixed, partial, expired, floor-lowering, unsigned, revoked or downgraded updates fail before successor execution.

The trusted installer stages content-addressed bytes in an inactive slot, verifies deterministic build identity and the complete update using the current verifier/minimal updater, runs update self-tests, and records pending successor state atomically. It boots the successor in quarantine. On signed health success it atomically commits successor package, roots and floors. On failed health, crash or timeout it atomically rolls back executable selection to the still-authorized predecessor while preserving the predecessor's monotonic floors and recording the failed successor nonce; the failed successor can never become active. Recovery from a committed bad update is a separately signed higher-sequence transition and may select older executable bytes without lowering sequence/floors. No defense is claimed if a privileged host administrator maliciously restores or rewrites the entire trusted state.

Conformance tests cover initial ceremony, valid predecessor/successor, deterministic reproduction, torn write, crash at every staging/quarantine/commit point, failed-update rollback, replay, sequence skip, downgrade, floor reduction, wrong predecessor, root-key transition, higher-sequence recovery and unauthorized update. All run within uncompromised trusted verifier/deployment state; they make no compromised-host claim.
