...
`root.json` has an exact closed JSON schema: object keys in this order only: `schemaVersion` (`"v3-boundary-root/1"`), `foundationParent` (40 lowercase hex), `artifactTree` (literal string `"EXTERNAL_ARTIFACT_TREE_OID"`), `exactEdgesSha256`, `foundationFilesSha256`, `inboundPolicySha256`, `auditContractSha256`, `certificationPayloadSha256`, `canonicalVectorsSha256` (each 64 lowercase hex). Duplicate, missing, extra, reordered, non-ASCII, escaped-key, alternate-number, whitespace outside the one canonical form, or non-shortest UTF-8 input is rejected before decoding. Repository `root.json` bytes are RFC 8785/JCS canonical JSON with those keys; hashes decode from hex to raw32.
