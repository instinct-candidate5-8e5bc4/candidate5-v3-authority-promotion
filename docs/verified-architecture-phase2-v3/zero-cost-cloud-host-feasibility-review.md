...
2. The exact UKI init requires two block devices already present as `/dev/v3-rootfs-data` and `/dev/v3-rootfs-hash`; it then constructs the exact read-only dm-verity table with root `b24dcf930604552f6624c87f1acb92069ca1bced98a726f29444ea4cb150ccb8`.
...
| genuine block devices | SECURITY-AUTHORITATIVE | Two block devices at exact paths `/dev/v3-rootfs-data` and `/dev/v3-rootfs-hash`; data image `5e8e7de6e6c405736ceb8ade688db540c29b840708e9d4224221ca283dc0fd25`; verity tree `d2e51f14b6c2b87cb1cc59d1a15e2ae6830ef8c8e6c22ed4550f840981a03394`. | Guest `stat` must prove block-device type; bytes/digests must match; swap, alias or ordinary-file substitution must fail. |
