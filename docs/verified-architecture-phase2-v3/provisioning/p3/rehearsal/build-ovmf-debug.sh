...
if [ "${CANON_MODE:-}" != hostile ] && [ "${IN_CANON_NS:-}" != 1 ]; then
  ...
  IN_CANON_NS=1 CANON_REALWORK="$REALWORK"     exec bwrap --ro-bind /usr /usr --symlink usr/bin /bin --symlink usr/sbin /sbin       --symlink usr/lib /lib --symlink usr/lib64 /lib64 --ro-bind /etc /etc       --bind /tmp /tmp --bind /home /home --bind /var /var --proc /proc --dev /dev       --dir /build --bind "$REALWORK" /build -- "$0" "$@"
fi
...
build_one() {
  ...
  cd "$WORK"
  git init -q edk2 && cd edk2
  git remote add origin https://github.com/tianocore/edk2.git
  git fetch -q --depth 1 origin "$EDK2_COMMIT"
  git checkout -q FETCH_HEAD
  local HEAD; HEAD=$(git rev-parse HEAD)
  [ "$HEAD" = "$EDK2_COMMIT" ] || { echo "E_EDK2_COMMIT_MISMATCH $HEAD"; exit 41; }
  for sm in $SUBMODULES; do git submodule update -q --init --depth 1 -- "$sm"; done
...
if [ "${CANON_MODE:-}" = hostile ]; then
  ...
  WORKFIX="$(dirname "$OUTA")/ovmf-hostile"
