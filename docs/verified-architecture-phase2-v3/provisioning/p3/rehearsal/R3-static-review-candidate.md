...
## 34. C1'''''''''''''''''''' revision (f48e8f0e pre-push review: privilege mismatch on the staging copies)

**Failure disclosure (caught pre-push this time).** The reviewer's pre-push ruling on f48e8f0e was NOT CLEARED on ONE blocking defect, same producer-to-consumer wiring class as run-36081199933: the ESP staging dirs (/tmp/$PREFIX-esp-a, /tmp/$PREFIX-esp-env2-a) are created by the SUDO'd builder (mkdir as root, umask 022 -> root:root 0755, never chowned), so f48e8f0e's post-build copies into them ran WITHOUT sudo as the runner user and would have died EACCES under set -e BEFORE the E_SLOT_ESP_COPY_PIN check could fire - in both the main ESP step and env2. test-esp-fresh-dirs.py could not see it: its stub sequence strips the sudo wrapper and runs single-user, so ownership is invisible to the executed green.

**34a. The fix (the ruling's sudo-prefix option).** Both copies are now `sudo cp --sparse=always <slot-a product> <staging target>` in the rehearsal workflow's main ESP step and env2 step, with E_SLOT_ESP_COPY_PIN immediately after each (the runner reads the pin afterwards unprivileged; reading needs no sudo). The ALLOWED ALTERNATIVE (repoint run-ceremony/qemu-smoke/env2 consumers to the fresh slot dirs, no copy) was weighed and rejected: those consumers were byte-verified as cleared at 41ccb99c, and the sudo-prefix keeps them byte-identical. The scratch workflow carries both sudo copies by derivation (2,057 lines, zero-diff vs a fresh derive-scratch.py run). Nothing else changed.

**34b. The structural ownership proof.** test-esp-fresh-dirs.py grew a structural section per the ruling: it extracts every builder work dir from the ESP-step bodies (8 per lane, fail-closed), finds every command writing INTO one of those dirs (exactly the 2 staging copies per lane, fail-closed), and requires each to be sudo-prefixed. Planted negative: stripping sudo from the copies (the exact f48e8f0e shape) is RED. TRUE-RED executed proof: the committed test run against the REAL f48e8f0e workflow files (swapped in from git, then restored) failed exactly the two sudo-prefix checks in both lanes, naming both non-sudo copies; against the fixed workflows all 18 checks pass. The executed stub sequence now also strips `sudo cp` -> `cp` in its single-user sandbox, with a comment pointing at the structural section as the ownership proof.

**34c. Validation inventory.** All 19 suites green (test-esp-fresh-dirs 18/18 with the structural section). Executed preflight under BOTH lanes on the final tree: the error set EQUALS the 173-entry baseline EXACTLY {E_LOCK_MISSING_DEB x161, E_STAGED_TOOL_MISSING x10, E_BWRAP_ARGV_MISMATCH x1, E_VARS_MISSING x1}, exit 30. 6e6 prefix-count pin unchanged at 85 (no new lane-prefix literals; verified live). YAML parses for all three workflows; bash -n clean; zero tracked bytecode; exec bits 100755; derive-identity zero-diff. argv-freeze.json untouched (sha256 97b14012..); certification workflow byte-identical. A3 manifest re-pinned; declaration updated (this head is a 6-path delta on its parent f48e8f0e: the 2 workflows, the structural test, this file, the manifest, and the regenerated diff). certification-vs-rehearsal.diff regenerated by its pinned recipe (the sudo edit changed the rehearsal workflow text, so the f48e8f0e-era diff was stale). Mishaps disclosed: (i) the f48e8f0e validation gap (single-user stub execution invisible to ownership) is exactly the gap the ruling named; the structural section closes it, and the true-RED swap-proof is recorded above; (ii) bc6a1c7c shipped a STALE certification-vs-rehearsal.diff (byte-identical to f48e8f0e's, still documenting the unsudoed copies at diff lines 197/567 while the fixed workflow carries sudo cp): the sudo edit obligated a regeneration by the pinned recipe and it was missed at delivery. Caught by the producer's own report-writing consistency check AND confirmed by main-side verification before any relay; the flawed SHA was discarded locally (never pushed, never relayed, never entered review) and is REPLACED by this commit at the same lineage position (same parent f48e8f0e, same 20-prime designation, same fix content plus the regenerated diff and this disclosure).
