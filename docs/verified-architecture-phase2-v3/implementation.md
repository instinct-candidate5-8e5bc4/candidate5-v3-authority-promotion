...
Narrow phase uses canonical axis keys and exact rational SAT projections for boxes/convexes. Zero cross axes are dropped. Axes and compound pairs are sorted. Touching counts as collision for BODY_VS_BODY. Convex definitions require at least four vertices/faces, valid unique face indices, nondegenerate faces, supplied physical normals, and a same-half-space convexity check independent from visuals.
